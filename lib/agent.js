 ...
 * Draws lines every `step` pixels with coordinate labels.
 * Line color: bright green. Label background: black.
 */
  ...
  async run(task, opts = {}) {
    ...
    for (let step = 1; step <= this._maxSteps; step++) {
      ...
      this._log(1, `  📷 screenshot ${imgW}×${imgH}` +
        ...
        (this._gridStep ? ` [grid ${this._gridStep}px]` : '') +
      ...
      if (this._verbose >= 1) {
        for (let i = 0; i < actions.length; i++) {
          ...
          if (a.x !== undefined && a.y !== undefined) {
            if (coordsScaled) {
              coords = ` img(${ah.x},${ah.y}) → real(${a.x},${a.y})`;
            } else {
              coords = ` (${a.x},${a.y})`;
            }
          }
