...
class Agent {
  ...
  async run(task, opts = {}) {
    ...
    // Track every action key that has been executed to detect repeats.
    // Key format: "cmd:x:y" for pointer actions, "cmd:text" for type/key.
    ...
    for (let step = 1; step <= this._maxSteps; step++) {
      ...
      if (step > 1 && history.length > 0) {
        if (!changed) {
          ...
          history[history.length - 1].note = 'Screen unchanged after these actions — try a completely different approach';
        } else {
          this._log(1, `  ✓  Screen changed after last step`);
          history[history.length - 1].note = 'Screen changed — evaluate whether this reached the intended state';
        }
      }
      ...
      let imgForAI = this._maxImageWidth > 0
        ...
        : screenshot;
      // Grid overlay (disabled by default; enable via options.gridStep for debugging).
      // if (this._gridStep > 0) {
      //   imgForAI = overlayGridPNG(imgForAI, this._gridStep);
      // }
      const { width: imgW, height: imgH } = _decodePNG(imgForAI);
      // Save the image that will actually be sent to the AI (resized)
      this._saveScreenshot(imgForAI, `step-${step}-ai.png`);
      // Scale AI coordinates to real screen pixels.
      // 'norm1000': Qwen2-VL/Qwen3-VL outputs 0-1000 normalised coords.
      // 'pixels':   model outputs pixel coords of the image as sent.
      ...
      const coordLabel = isNorm
        ? 'coords 0-1000 \u2192 real'
        : `coords pixels(${imgW}\u00d7${imgH}) \u2192 real`;
      this._log(1, `  \ud83d\udcf7 screenshot ${imgW}\u00d7${imgH}` +
        ` (real ${realW}\u00d7${realH}, ${coordLabel})` +
        ` \u2192 AI (${(imgForAI.length / 1024).toFixed(0)} KB)`);

      // ── 2. Build prompt ─────────────────────────────────────────────────
      // Remind the model which coordinate system to use.
      const coordNote = isNorm
        ? '\nCoordinates are 0-1000 normalised: (0,0)=top-left, (1000,1000)=bottom-right.'
        : `\nCoordinates are pixel values of the image you see (width: ${imgW}, height: ${imgH}).`;
      ...
      // Mark repeated actions and register all new ones in triedActionKeys.
      for (const a of actionsForHistory) {
        const key = a.x !== undefined
          ? `${a.cmd}:${a.x}:${a.y}`
          : `${a.cmd}:${a.text || a.combo || ''}`;
        if (triedActionKeys.has(key)) {
          a._repeated = true;
        } else {
          triedActionKeys.add(key);
        }
      }
      // note is filled in at the start of the NEXT iteration once we can compare screenshots
      history.push({ step, actions: actionsForHistory, note: 'outcome pending' });
    }
