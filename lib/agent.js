...
class Agent {
  ...
  async run(task, opts = {}) {
    ...
    for (let step = 1; step <= maxSteps; step++) {
      ...
      const pendingSaves = [];
      if (this._screenshotDir) {
        // initiate save but do not await yet
        pendingSaves.push(
      ...
      // Get AI response – build messages inline rather than calling helpers
      const userText = buildTaskMessage(task, step, history);
      const png = resized instanceof ImageRaw ? resized.encodePNG() : resized;
      const userContent = [
        { type: 'image_url', image_url: { url: png.encodeForModel() } },
