...
class Agent {
   ...
   */
  // Return the raw RGBA framebuffer synchronously. Previously this was an
  // async method, but callers (notably {@link Agent.run}) always awaited it
  // immediately, so the `async` keyword was unnecessary and prevented us from
  // performing non-blocking work around the callsite. Keeping it sync lets the
  // main loop capture the image and then kick off PNG encoding/saving without
  // waiting.
  screenshotRaw() {
    // gracefully handle being called before a screen buffer exists; this mirrors
    // the documented behaviour of returning `null` until an update arrives.
    if (!this._screenBuffer || typeof this._screenBuffer.captureScreen !== 'function') {
      return null;
    }
  ...
  async run(task, opts = {}) {
    ...
    for (let step = 1; step <= maxSteps; step++) {
      ...
      if (step > 1) {
        const currentCount = this._screenBuffer?.updateCount ?? lastUpdateCount;
        const changed = currentCount - lastUpdateCount > 0;
        if (changed) {
      ...
      // snapshot updateCount before we execute actions so next iteration can compare
      ...
      // ── Stage image + task text, then send ────────────────────────────────
      // No history argument needed: the chat session already holds the full
      // conversation, so buildTaskMessage is called with an empty history.
      const pngBuf  = (resized instanceof ImageRaw ? resized.encodePNG() : resized).buffer;
      const stepMsg = buildTaskMessage(task, step);
      aiChat.userImage(pngBuf, 'image/png');
      ...
      // update baseline for next iteration's outcome check (after actions
      // have settled; waitForScreenChange may have advanced the counter)
      lastUpdateCount = this._screenBuffer?.updateCount ?? lastUpdateCount;
    }
