...
class Agent {
   ...
   */
  async waitForScreenChange(baselinePng, { maxWait = 3000, pollInterval = 400, minChange = 1 } = {}) {
    ...
    while (Date.now() < deadline) {
      ...
      if (this._screenBuffer) {
        ...
        if (delta < minChange) {
          // not enough updates yet – skip the expensive screenshot call
          continue;
        }
        // enough updates have happened
      }
      // framebuffer updated (or we don't have a buffer); grab a fresh copy and
      // return immediately.  The updateCount delta is considered sufficient
      // evidence that something meaningful has changed; we keep the same simple
      // boolean return value so callers don't need to care about the numeric
      // delta.
  ...
  async run(task, opts = {}) {
    ...
    let history             = [];
    let lastUpdateCount     = null;        // track for outcome annotation
    let finalResult         = { steps: maxSteps, result: 'Max steps reached' };

    for (let step = 1; step <= maxSteps; step++) {
      const rawShotObj = await this.screenshotRaw();
      if (!rawShotObj) {
        // If we can't get a framebuffer there's no point continuing; it's
        // usually indicative of a dropped connection or other fatal VNC
        // failure.  Previously we silently continued with an empty buffer,
        // which merely hid the underlying bug and let tests pass.  Fail fast
        // instead so callers can detect the problem.
        throw new Error('Unable to capture screenshot (VNC connection lost?)');
      }
      const screenshot = rawShotObj.rgba;
      this.saveScreenshot(screenshot, `step-${step}-pre.png`);

      // annotate the outcome of the previous action using the screen buffer's
      // updateCount rather than an expensive pixel comparison.
      if (lastUpdateCount !== null) {
        const currentCount = this._screenBuffer ? this._screenBuffer.updateCount : 0;
      ...
      for (const a of actions) {
        if (a.minChange !== undefined) {
          const hint = parseFloat(a.changeFactor);
      ...
      minChange ||= 0.1;  // default to waiting for at least 10% screen change

      const { screenshot: postPng, changed } = await this.waitForScreenChange(screenshot, { minChange });
      this.saveScreenshot(postPng, `step-${step}-post.png`);

      // record the latest update count for the next iteration's outcome check
      lastUpdateCount = this._screenBuffer ? this._screenBuffer.updateCount : lastUpdateCount;
