...
class Agent {
   ...
   * width*height*4. This is a cheap getter (no PNG work) and is useful when you just
  ...
  /**
   * Throw a clear error if the agent hasn't been connected yet.
   * This is called at the start of public methods that require an active
   * connection so callers don't need to repeatedly guard individual fields.
   *
   * We deliberately avoid creating a full mock object here; instead we just
   * ensure the fundamental pieces exist and surface a descriptive message.
   */
  _ensureConnected() {
    if (!this._vnc) {
      throw new Error('Agent is not connected; call connect() before run()');
    }
    if (!this._screenBuffer) {
      throw new Error('Agent has no screen buffer; ensure connect() completed');
    }
  }
   ...
   * @returns {Promise<boolean>} true if a change was detected, false on timeout
   */
  async waitForScreenChange({ maxWait = 3000, pollInterval = 400, minChange} = {}) {
    const deadline = Date.now() + maxWait;

    while (Date.now() < deadline) {
      await _sleep(pollInterval);
      if (minChange !== undefined && this._screenBuffer.updateCount > minChange)
        return true;
    }
  ...
  /**
   * Extract action objects from a free-form AI response string.
   *
   * The model often embeds JSON objects inside prose or markdown fences. This
   * helper finds every `{...}` fragment in the text, attempts to parse each one
   * as JSON, and applies a lightweight repair pass on failures (unquoted keys,
   * missing `"y"` fields, trailing commas). If parsing ultimately fails the
   * returned entry is `{cmd:'unknown'}` so the caller can continue processing
   * without crashing.
   *
   * @param {string} responseText  Raw assistant response text
   * @returns {Array<object>}  Array of action objects (possibly empty)
   */
  extractActions(responseText) {
    const matches = responseText.matchAll(/\{[^}]+\}/g);
    const actions = matches ? Array.from(matches).map(m => {
      try {
  ...
  processAIResponse(responseText, img) {
    // quick-path: try the lightweight extractor which simply finds any
    // `{...}` fragments. If the model only returned a single done object we
    // can avoid the more heavyweight parsing logic and exit early. This mirrors
    // the behaviour requested by the user comment: "use the simple extract
    // actions first, if the return is not [{done:true,result:"summary"}] then
    // we can scale them properly inside the loop".
    const simple = this.extractActions(responseText);
    if (simple.length === 1 && simple[0].done === true) {
      return { done: true, result: simple[0].result || 'Done' };
    }

    let rawActions;
    if (simple.length > 0) {
      // we found at least one non-done action via the simple extractor; use it
      rawActions = simple;
    } else {
      // fallback to the full parser which handles fenced JSON, arrays, etc.
      const parsed = parseResponse(responseText);
      if (parsed.done) {
        return { done: true, result: parsed.result || 'Done' };
      }
      rawActions = Array.isArray(parsed) ? parsed : [parsed];
    }
   ...
   */
  async run(task, opts = {}) {
    // require a live connection before beginning the loop; this avoids
    // obscure null dereferences later and makes the error easier to diagnose.
    this._ensureConnected();

    const maxSteps = opts.maxSteps || this._maxSteps;
    const onStep   = opts.onStep || null;

    // Create a fresh chat session for this task. The chat maintains the full
    // message history (including images and assistant replies) and submits it
    // on every send(). Model-side KV-cache handles the repeated prefix so we
    // don't need to summarise or truncate history ourselves.
    const aiChat = this._ai.newChat();
    aiChat.systemText(this._systemPrompt);
    aiChat.userText(`TASK: ${task}`);
    ...
    for (let step = 1; step <= maxSteps; step++) {
      // capture raw screenshot synchronously; null means fatal error
      const screenshot = this.screenshotRaw();
      if (!screenshot) throw new Error('Unable to capture screenshot');

      // collect any pending file-write promises so we can await them later
      const pendingSaves = [];
      if (this._screenshotDir) {
        pendingSaves.push(
          screenshot.encodePNG().save(path.join(this._screenshotDir, `step-${step}.png`))
        );
      }
      ...
      //let cursorX = this._mouseX;
      //let cursorY = this._mouseY;
      let resized = cropRect ? screenshot.crop(cropRect) : screenshot;
      //if (this._maxImageWidth > 0) {
      //  resized = resized.resize(this._maxImageWidth);
        //cursorX = Math.round(this._mouseX * (resized.width / this._vnc.width));
        //cursorY = Math.round(this._mouseY * (resized.height / this._vnc.height));
      //}
      //resized.drawCursor(cursorX, cursorY);
      //if (this._gridStep > 0) resized.overlayGrid(this._gridStep);

      //if (this._screenshotDir) {
      //  pendingSaves.push(
      //    resized.encodePNG().save(path.join(this._screenshotDir, `step-${step}-ai.png`))
      //  );
      //}
      //this.logStep(step, resized);

      // ── Build coordinate metadata ─────────────────────────────────────────
      //const isNorm = this._coordSystem === 'norm1000';
      //const scaleX = isNorm ? this._vnc.width / 1000 : this._vnc.width / resized.width;
      //const scaleY = isNorm ? this._vnc.height / 1000 : this._vnc.height / resized.height;
      //const imgMeta = { width: resized.width, height: resized.height, scaleX, scaleY, isNorm };

      //const pngBuf  = (resized instanceof ImageRaw ? resized.encodePNG() : resized).buffer;
      //const stepMsg = buildTaskMessage(task, step);
      const stepMsg = `Screenshot original dimentions: ${screenshot.width}×${screenshot.height}.
        ${cropRect ? `Crop region: ${JSON.stringify(cropRect)}` : 'No crop region selected.'}`;

      const png = resized.encodePNG();
      if (this._screenshotDir) {
        pendingSaves.push(
          png.save(path.join(this._screenshotDir, `step-${step}-ai.png`))
        );
      }
      aiChat.userImage(png.encodeForModel());
      aiChat.userText(stepMsg);
      ...
      //this.logPrompt(this._systemPrompt, aiChat._pending);

      const responseText = await aiChat.send();
      await Promise.all(pendingSaves);
      this.logResponse(step, responseText);

      // parse the assistant reply and decide what to do next. we used to
      // call `extractActions()` directly here, but that helper only returns a
      // flat list of objects and can't signal completion or perform scaling.
      // `processAIResponse` wraps `parseResponse` and provides the richer
      // `{done,actions,result}` shape that the loop expects.
      let actions = this.extractActions(responseText);
      if (actions.length === 1 && actions[0].done === true) {
        finalResult = { steps: step, result: actions[0].result };
        break;
      }
      ...
      actions = actions.filter(a => {
        if (a.cmd === 'crop') {
          if (a.x !== undefined && a.y !== undefined && a.width !== undefined && a.height !== undefined) {
            cropRect = { x1: a.x, y1: a.y, x2: a.x2, y2: a.y2 };
            this._log(1, `  🗔  Crop command received: ${JSON.stringify(cropRect)}`);
          } else {
            cropRect = false; // reset to no crop if invalid
            this._log(1, '  🗔  Crop reset received');
          }
          return false; // don't treat crop as an actionable command
        }
        return true; // keep all non-crop commands
      });
      //actions = this.scaleActions(actions, imgMeta);
      ...
      // Execute actions and wait for the screen to reflect them
      await this.execute(actions);
      const minChange = this.calculateMinChange(actions) + lastUpdateCount;
      const changedInsignificantly = await this.waitForScreenChange({ minChange });

      // ── Outcome annotation ────────────────────────────────────────────────
      // Inform the model about whether the previous action had a visible
      // effect. We stage this as a text part of the *current* user message so
      // it arrives alongside the new screenshot.
      const currentUpdateCount = this._screenBuffer.updateCount;
      const changed = currentUpdateCount != lastUpdateCount;

      if (changedInsignificantly) {
        this._log(1, '  ⚠  Screen changed but below expected threshold — action may have had limited effect');
        aiChat.userText('OUTCOME: Screen changed but below expected threshold — evaluate whether it partially matches your expected result or if you need a different approach.');
      } else if (changed) {
        this._log(1, '  ✓  Screen changed');
        aiChat.userText('OUTCOME: Screen changed — evaluate whether it matches your expected result.');
      } else {
        this._log(1, '  ⚠  Screen unchanged — action had no visible effect');
        aiChat.userText('OUTCOME: Screen did NOT change — the action had no visible effect. You MUST try a different approach.');
      }

      lastUpdateCount = this._screenBuffer.updateCount;
    }
