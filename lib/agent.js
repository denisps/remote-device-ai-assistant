...
const { resizePNG, overlayGridPNG, decodePNG, renderCursorPNG } = require('./image');
...
class Agent {
   ...
   * Also renders the cursor at the current mouse position.
  ...
  prepareImage(pngOrRaw) {
    ...
    buffer = renderCursorPNG(buffer, cursorX, cursorY);
  ...
  async execute(actions) {
    for (const action of actions) {
      ...
      if (cmd === 'move' || cmd === 'click') {
        ...
        // If this is a click, execute it now at the current mouse location
        if (cmd === 'click') {
          await this._vnc.run([{
            cmd: 'click',
        ...
        await this._vnc.run([{ cmd: 'scroll', h, v }]);
  ...
  /**
   * Capture a screenshot for this step and validate it's available.
   * @param {number} step  Current step number
   * @returns {Promise<Buffer>}  RGBA buffer
   * @throws {Error} if screenshot cannot be captured
   */
  async captureStepScreenshot(step) {
  ...
  async run(task, opts = {}) {
    ...
    for (let step = 1; step <= maxSteps; step++) {
      ...
      // Annotate outcome from previous step
      ...
      // Prepare image with cursor rendered
