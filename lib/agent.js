...
const { resizePNG, overlayGridPNG, decodePNG, scaleRGBA, drawCursor, overlayGrid } = require('./image');
...
class Agent {
  ...
  /**
   * Prepare raw RGBA screenshot for AI: resize, render cursor, optionally overlay grid.
   * Works entirely with RGBA buffers - no PNG encoding/decoding.
   * Returns raw RGBA along with dimensions and coordinate scale factors.
   * 
   * @param {Buffer} rgba  Raw RGBA buffer (width * height * 4 bytes)
   * @returns {{ rgba: Buffer, realW, realH, imgW, imgH, scaleX, scaleY, isNorm }}
   */
  prepareImage(rgba) {
    const realW = this._vnc.width;
    const realH = this._vnc.height;
    
    // Verify we received raw RGBA
    if (!rgba || rgba.length !== realW * realH * 4) {
      throw new Error(`prepareImage expects raw RGBA buffer of size ${realW * realH * 4}, got ${rgba?.length || 0}`);
    }

    let imgW = realW;
    let imgH = realH;
    let processedRGBA = rgba;

    // Resize if needed
    if (this._maxImageWidth > 0 && imgW > this._maxImageWidth) {
      const dstW = this._maxImageWidth;
      const dstH = Math.round(realH * dstW / realW);
      processedRGBA = scaleRGBA(rgba, realW, realH, dstW, dstH);
      imgW = dstW;
      imgH = dstH;
    }

    // Render cursor at current position (scaled to image dimensions)
    const cursorScaleX = imgW / realW;
    const cursorScaleY = imgH / realH;
    const cursorX = Math.round(this._mouseX * cursorScaleX);
    const cursorY = Math.round(this._mouseY * cursorScaleY);
    drawCursor(processedRGBA, imgW, imgH, cursorX, cursorY);

    // Overlay coordinate grid if requested
    if (this._gridStep > 0) {
      overlayGrid(processedRGBA, imgW, imgH, this._gridStep);
    }
    ...
    return { rgba: processedRGBA, realW, realH, imgW, imgH, scaleX, scaleY, isNorm };
  }
   ...
   * Accepts either raw RGBA buffer or pre-encoded PNG buffer.
   * If RGBA is provided, encodes it to PNG before saving.
   * 
   * @param {Buffer} data      Raw RGBA buffer or PNG buffer
   * @param {string} filename  Filename (should include .png extension)
   * @param {number} [width]   Required if data is RGBA buffer
   * @param {number} [height]  Required if data is RGBA buffer
   */
  saveScreenshot(data, filename, width, height) {
    if (!this._screenshotDir) return;
    
    let png = data;
    // If dimensions provided, treat as raw RGBA and encode
    if (width && height) {
      if (data.length !== width * height * 4) {
        throw new Error(`RGBA buffer size mismatch: expected ${width * height * 4}, got ${data.length}`);
      }
      png = encodePNG(width, height, data);
    } else if (this._vnc && data.length === this._vnc.width * this._vnc.height * 4) {
      // Fallback: heuristic check if it matches current VNC dimensions
   ...
   * Encodes the RGBA buffer to PNG for transmission.
   * 
   * @param {string} task
   * @param {number} step
   * @param {Array}  history
   * @param {{ rgba, imgW, imgH, isNorm }} img  Result of prepareImage()
  ...
  buildMessages(task, step, history, img) {
    ...
    // Encode RGBA to PNG for AI transmission
    const pngBuffer = encodePNG(img.imgW, img.imgH, img.rgba);
    
    return [
      { role: 'system', content: this._systemPrompt },
      this._ai.buildImageMessage(pngBuffer, userText),
    ];
  }
  ...
  logStep(step, img) {
    const rgbaKB = (img.rgba.length / 1024).toFixed(0);
    this._log(1, `  📷 step ${step}/${this._maxSteps}: ${img.imgW}×${img.imgH}` +
      ` (real ${img.realW}×${img.realH}) RGBA: ${rgbaKB} KB`);
  }
   ...
   * @returns {Promise<Buffer>}  RGBA buffer
  ...
  async run(task, opts = {}) {
    ...
    for (let step = 1; step <= maxSteps; step++) {
      ...
      this.saveScreenshot(screenshot, `step-${step}.png`, this._vnc.width, this._vnc.height);
      ...
      // Prepare image: resize, draw cursor, overlay grid (all in RGBA)
      const img = this.prepareImage(screenshot);
      this.saveScreenshot(img.rgba, `step-${step}-ai.png`, img.imgW, img.imgH);
      ...
      // Process response
      const result = this.processAIResponse(responseText, img);
      if (result.done) {
