...
class Agent {
  ...
  async screenshotRaw() {
    ...
    // Try the raw getter/method first.  Older vnc-tool versions may not have it
    // or it may return null until the first framebuffer update arrives.
    let raw;
    if (typeof this._vnc.screenshotRaw === 'function') {
      raw = this._vnc.screenshotRaw();
    } else {
      raw = this._vnc.screenshotRaw;
    }
    if (raw) {
      return { width: this._vnc.width, height: this._vnc.height, rgba: raw };
    }

    // Fallback: grab a PNG and decode it.  This is the expensive path and only
    // exercised when the client does not yet expose the raw buffer.
    try {
  ...
  async waitForScreenChange(baselinePng, { maxWait = 3000, pollInterval = 400 } = {}) {
    // If the underlying VNC client supports updateCount we can avoid taking
    // a new screenshot on every poll.  A non‑zero delta means something has
    // updated in the framebuffer; we still grab one PNG when that happens so
    // we can compare it with the baseline and possibly return it to callers.
    const startCount = this._vnc ? this._vnc.updateCount : 0;
    const deadline = Date.now() + maxWait;

    while (Date.now() < deadline) {
      await _sleep(pollInterval);
      if (this._vnc && this._vnc.updateCount === startCount) {
