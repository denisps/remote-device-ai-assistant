...
class Agent {
  ...
  /**
   * Validate an AI configuration by making a minimal chat request.
   *
   * @param {object} aiConfig
   * @param {string} [aiConfig.baseUrl]
   * @param {string} [aiConfig.apiKey]
   * @param {string} [aiConfig.model]
   * @returns {Promise<{ok:boolean,response?:string,error?:string}>}
   */
  static async validateAI(aiConfig) {
  ...
  /**
   * Validate a VNC configuration by connecting, taking a screenshot, then disconnecting.
   *
   * @param {object} vncConfig
   * @param {string} [vncConfig.host]
   * @param {number} [vncConfig.port]
   * @param {string} [vncConfig.password]
   * @returns {Promise<{ok:boolean,width?:number,height?:number,error?:string}>}
   */
  static async validateVNC(vncConfig) {
   ...
   * @param {number} [options.minChange=1]      required updateCount
   * @returns {Promise<boolean>} true if a change was detected, false on timeout
   */
  async waitForScreenChange({ maxWait = 3000, pollInterval = 400, minChange = 0} = {}) {
    const deadline = Date.now() + maxWait;

    while (Date.now() < deadline) {
      await _sleep(pollInterval);
      if (minChange > 0 && this._screenBuffer.updateCount > minChange)
        return true;
    }
  ...
  async run(task, opts = {}) {
    ...
    for (let step = 1; step <= maxSteps; step++) {
      if (step > 1) {
        ...
        // Screenshot is a local action
        actions = actions.filter(async a => {
          if (a.cmd === 'crop') {
        ...
        try {
          ...
          this._log(1, `  ⚠  Error executing actions: ${error.message}`);
          aiChat.userText(`OUTCOME: Error executing actions: ${error.message} — the action may have had no effect. You MUST try a different approach.`);
          lastUpdateCount = this._screenBuffer.updateCount;
          continue; // skip waiting for screen change and move to next step
        }
        
        await _sleep(1000);

        const expectedChange = this.calculateMinChange(actions);
        const waitOptions = {};
        if (expectedChange > 0) {
          this._log(1, `Expected change: ${expectedChange.toFixed(2)} (based on action hints)`);
          waitOptions.minChange = expectedChange + this._screenBuffer.updateCount;
        }

        const changedInsignificantly = await this.waitForScreenChange(waitOptions);
        ...
        if (changedInsignificantly) {
          ...
          continue; // skip screenshot and move to next step
        }
      }
      ...
      screenshot.drawCursor(this._mouseX, this._mouseY);
      if (this._gridStep > 0) screenshot.drawGrid(this._gridStep);

      const resized = cropRect ? screenshot.crop(cropRect) : screenshot;

      const png = await resized.encodePNG();

      if (this._screenshotDir) await png.save(path.join(this._screenshotDir, `step-${step}.png`));
