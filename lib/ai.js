...
class AIChat {
   ...
   * Returns `this` for chaining.
   *
   * @param {Image} image    Image object with encodeForModel() method that returns a data URL string
  ...
  async send() {
    ...
    // Always send the pending array verbatim.  Previous versions collapsed a
    // single text part to a plain string for compatibility with some model
    // servers, but that behaviour has been removed; callers should now handle
    // the array form if necessary.
