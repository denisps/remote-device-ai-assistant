...
class AIChat {
  ...
  constructor(client) {
    ...
    this._pendingSystem = []; // staged system messages (flushed on send)
  }
   ...
   */
  systemText(text) {
    // Stage system instructions so they can be flushed together with the
    // next send() call, just like user text/images.
    this._pendingSystem.push(text);
    return this;
  }
  ...
  /**
   * Stage an image URL (or data URL) for the next user message.
   * Returns `this` for chaining.
   *
   * @param {string} url    Image URL or data URL (e.g. "data:image/png;base64,...")
   */
  userImage(url) {
   ...
   */
  async send() {
    if (this._pendingSystem.length === 0 && this._pending.length === 0)
      return;

    // Flush staged system messages first (if any).
    if (this._pendingSystem.length > 0) {
      for (const text of this._pendingSystem) {
    ...
    if (this._pending.length > 0) {
      this._history.push({ role: 'user', content: this._pending });
      this._pending = [];
    }
  ...
  /**
   * Create an independent clone of this chat session.
   * The returned chat shares the same AIClient but has its own history/pending state.
   * @returns {AIChat}
   */
  fork() {
    const clone = new AIChat(this._client);

    const cloneArray = (arr) => JSON.parse(JSON.stringify(arr));

    clone._history = cloneArray(this._history);
    clone._pending = cloneArray(this._pending);
    clone._pendingSystem = cloneArray(this._pendingSystem);
...
class AIClient {
  ...
  async _request(url, body) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`API responded with error: ${parsed.error?.message || `HTTP ${res.status}`}`);
      }
      return parsed;
    } finally {
      clearTimeout(timer);
