...
/** Build the synthetic test image (white background, red square at 75%,75%). */
function _buildTestImage() {
  ...
  return ImageRaw.fromRaw(DETECT_W, DETECT_H, rgba).encodePNG();
}
 ...
 */
async function detectCoordSystem(aiConfig) {
  const ai = new AIClient(aiConfig);
  const png = _buildTestImage();
