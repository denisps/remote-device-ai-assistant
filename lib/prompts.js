...
COMMAND FORMAT: json with the command itself in the "cmd" field, and any arguments as additional fields.
EXAMPLE: {"cmd":"move","x":500,"y":300}

SUPPORTED COMMANDS:
"click" – click at the current mouse pointer. Use move/dmove first to reposition if needed.
  Optional argument: button ("left"|"right"|"middle", default "left").
"move" – move mouse to absolute position (animated).
  Arguments: x, y (numbers, 0‑1000 normalised or pixels depending on coord system).
"dmove" – move mouse by relative delta (animated).
  Arguments: dx, dy (numbers, positive or negative change).
"type" – type text at current cursor position.
  Argument: text (string).
"key" – send keyboard shortcut or special key.
  Argument: combo (string, e.g. "Return", "Ctrl+c").
"scroll" – scroll relative to current position.
  Arguments: h (horizontal delta), v (vertical delta). Both default to 0.
  Example: {"cmd":"scroll","v":3}  # scroll down
  Example: {"cmd":"scroll","h":-2,"v":1}
"delay" – wait before continuing.
  Arguments: ms (number, milliseconds).
  Optional: minChange (number 0‑1, hint for expected screen change).

TASK COMPLETION:
When the task is complete, respond with ONLY: {"done":true,"result":"one-sentence summary"}

JSON FORMAT:
Embed actions as valid JSON objects or arrays in your response text. Examples:
