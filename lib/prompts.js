...
Command "move" - move mouse to absolute position.
  Arguments: x, y (numbers, 0-1000 normalised or pixels depending on coord system).
Command "dmove" - move mouse by relative delta.
...
Command "crop" - set a crop region or reset to a full screenshot if no arguments are provided.
  Arguments: x1, y1, x2, y2 (numbers, coordinates of the top-left and bottom-right corners of the crop region).
...
RULES:
- Do not stop the VNC server your are using to interact with the device.
- All absolute positions are in the original screenshot's coordinate system.
...
- Don't click unless you moved the mouse pointer and/or verified its position first. Use the mouse cursor as your guide.
- Don't guess UI element positions. Before clicking a small target, first send a crop command which should zoom it into your viewport.
- If you can't find a reliable target, try a different approach (e.g. keyboard shortcuts, context menu, etc).
- When the task objective is met, immediately return the done signal. Do not keep acting.
- You must translate your coordinates into the screen dimentions.
`;
