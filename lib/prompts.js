...
function buildTaskMessage(task, step = 1, history = []) {
  ...
  if (history.length > 0) {
    ...
    for (const entry of history) {
      const actSummary = entry.actions
        .map(a => {
          ...
          const repeated = a._repeated ? ' ⚠ ALREADY TRIED BEFORE — DO NOT REPEAT' : '';
          return `    ${a.cmd}${coords}${text}${reason}${repeated}`;
        })
        .join('\n');
      msg += `  Step ${entry.step}:\n${actSummary}\n`;
      if (entry.note && entry.note !== 'outcome pending') msg += `    Outcome: ${entry.note}\n`;
    }
    ...
    msg += '  3. What is the best NEXT action — do NOT repeat any action marked ⚠ ALREADY TRIED.\n';
  } else {
